...
namespace Gov.Jag.PillPressRegistry.Interfaces
{
    ...
    static class Authentication
    {
        ...
        private static string WrapInSoapMessage(string stsResponse, string relyingPartyIdentifier)
        {
            ...
            var requestedTokenRaw = samlAssertion.GetElementsByTagName("t:RequestedSecurityToken")[
                0
            ];
            ...
            XmlElement soapEnvelope = soapMessage.CreateElement(
                "t",
                "RequestSecurityTokenResponse",
                "http://schemas.xmlsoap.org/ws/2005/02/trust"
            );
            soapMessage.AppendChild(soapEnvelope);
            XmlElement lifeTime = soapMessage.CreateElement(
                "t",
                "Lifetime",
                soapMessage.DocumentElement.NamespaceURI
            );
            soapEnvelope.AppendChild(lifeTime);
            XmlElement created = soapMessage.CreateElement(
                "wsu",
                "Created",
            ...
            XmlElement expires = soapMessage.CreateElement(
                "wsu",
                "Expires",
                "http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd"
            );
            ...
            XmlElement appliesTo = soapMessage.CreateElement(
                "wsp",
                "AppliesTo",
                "http://schemas.xmlsoap.org/ws/2004/09/policy"
            );
            soapEnvelope.AppendChild(appliesTo);
            XmlElement endPointReference = soapMessage.CreateElement(
                "wsa",
                "EndpointReference",
                "http://www.w3.org/2005/08/addressing"
            );
            appliesTo.AppendChild(endPointReference);
            XmlElement address = soapMessage.CreateElement(
                "wsa",
                "Address",
                endPointReference.NamespaceURI
            );
            ...
            XmlElement requestedSecurityToken = soapMessage.CreateElement(
                "t",
            ...
            XmlElement tokenType = soapMessage.CreateElement(
                "t",
                "TokenType",
                soapMessage.DocumentElement.NamespaceURI
            );
            XmlText tokenTypeValue = soapMessage.CreateTextNode(
                "urn:oasis:names:tc:SAML:1.0:assertion"
            );
            tokenType.AppendChild(tokenTypeValue);
            soapEnvelope.AppendChild(tokenType);
            XmlElement requestType = soapMessage.CreateElement(
                "t",
                "RequestType",
                soapMessage.DocumentElement.NamespaceURI
            );
            XmlText requestTypeValue = soapMessage.CreateTextNode(
                "http://schemas.xmlsoap.org/ws/2005/02/trust/Issue"
            );
            requestType.AppendChild(requestTypeValue);
            soapEnvelope.AppendChild(requestType);
            XmlElement keyType = soapMessage.CreateElement(
                "t",
                "KeyType",
                soapMessage.DocumentElement.NamespaceURI
            );
            XmlText keyTypeValue = soapMessage.CreateTextNode(
        ...
        public static async Task GetFedAuth(
            string samlSite,
            string token,
        ...
        )
        {
            ...
            string stringData =
                $"wa=wsignin1.0&wctx={HttpUtility.UrlEncode(sharepointSite.Wctx)}&wresult={HttpUtility.UrlEncode(samlToken)}";
            ...
            if (
                !string.Equals(sharepointSite.Wreply, $"{cookieUri.Scheme}://{cookieUri.Authority}")
            )
            {
                ...
                cookieContainer.Add(
                    new Uri($"{cookieUri.Scheme}://{cookieUri.Authority}"),
                    new Cookie("FedAuth", fedAuthCookieValue, "/")
                );
            }
        }
        ...
        /// <returns></returns>
        private static string ParameterizeSoapRequestTokenMsgWithUsernamePassword(
            string url,
            string username,
            string password,
            string toUrl
        ...
        public static async Task<string> GetStsSamlToken(
            string spSiteUrl,
            string username,
            string password,
            string stsUrl,
