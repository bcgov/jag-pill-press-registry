...
{
    /// <summary>
    /// SharePoint file manager using Microsoft Graph API for SharePoint Online.
    /// </summary>
    public class CloudSharePointFileManager : ISharePointFileManager
    {
        ...
        public CloudSharePointFileManager(
        ...
        private async Task<string> GetDocumentLibraryIdByNameAsync(string listTitle)
        {
            ...
            string requestUrl =
                $"{GraphApiEndpoint}sites/{SiteId}/lists?$select=id,name,displayName";

            _logger.LogDebug(
                "GetDocumentLibraryIdByNameAsync - Request URL: {RequestUrl}",
                requestUrl
            );
        ...
        )
        {
            ...
            {
                item = new { name = fileName, fileSystemInfo = new { } }
            };
            ...
            while (uploadedBytes < totalBytes)
            {
                ...
                chunkContent.Headers.ContentType = new MediaTypeHeaderValue(
        ...
        {
            _logger.LogDebug(
                "DownloadFile - Downloading file: {ServerRelativeUrl}",
                serverRelativeUrl
            );
            ...
            {
                _logger.LogError(
                    "DownloadFile - Document library not found: {ListTitle}",
                    listTitle
                );
                throw new Exception($"Document library '{listTitle}' not found");
            }
            ...
            string encodedPath = string.Join(
                "/",
                decodedPath.Split('/').Select(Uri.EscapeDataString)
            );
            string requestUrl =
