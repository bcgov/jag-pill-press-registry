...
{
    public class OnPremSharePointFileManager : ISharePointFileManager
    {
        ...
        public OnPremSharePointFileManager(IConfiguration Configuration)
        {
            ...
            // Scenario #2 - Using an API Gateway with Basic Authentication.  The API Gateway will handle other authentication and have different credentials, which may be NTLM
            {
        ...
        )
        {
            ...
            fileDetailsList = fileDetailsList
                .Where(f => string.IsNullOrEmpty(documentType) || f.DocumentType == documentType)
                .ToList();
        ...
        public async Task<bool> RenameFile(string oldServerRelativeUrl, string newServerRelativeUrl)
        {
            bool result = false;
            string url =
                $"{ApiEndpoint}web/GetFileByServerRelativeUrl('{EscapeApostrophe(oldServerRelativeUrl)}')/moveto(newurl='{newServerRelativeUrl}', flags=1)";
